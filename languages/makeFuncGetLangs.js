...
if (!fs.existsSync(pathLanguageFile)) {
	log.warn("LANGUAGE", `تعذر العثور على ملف اللغة ${global.GoatBot.config.language}.lang، سيتم استخدام الملف الافتراضي "${__dirname}/en.lang"`);
...
function getText(head, key, ...args) {
	...
	if (typeof head == "object") {
		...
		head = head.head;
		if (!fs.existsSync(pathLanguageFile)) {
			log.warn("LANGUAGE", `تعذر العثور على ملف اللغة ${pathLanguageFile}، سيتم استخدام الملف الافتراضي "${path.normalize(`${__dirname}/en.lang`)}"`);
			pathLanguageFile = `${__dirname}/en.lang`;
		}
		const readLanguage = fs.readFileSync(pathLanguageFile, "utf-8");
		const languageData = readLanguage
